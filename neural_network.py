...
#     model.compile(loss='mean_absolute_error', optimizer='adam', metrics=["mae"], run_eagerly=True)  # Set run_eagerly=True
#     return model
...
# Enable eager execution for tf.data
...
def create_train_valid_set():
    ...
    nn_data = pd.read_sql_table("nn_data", engine)
    nn_data = nn_data.drop(["week", "product_cost", "product_max_bound"], axis=1)
    X_train, X_valid, y_train, y_valid, product_encoder_train, product_encoder_valid = get_data_splits(nn_data)

    return X_train, X_valid, y_train, y_valid, product_encoder_train, product_encoder_valid
...
def get_data_splits(dataframe, valid_fraction=0.2):
    ...
    # Drop 'product_id' from features
    X_train = train.drop(['product_id'], axis=1)
    X_valid = valid.drop(['product_id'], axis=1)
    
    # Extract 'product_id' as labels
    y_train = train["product_id"]
    y_valid = valid["product_id"]
    
    # Encode 'product_id'
    product_encoder_train = LabelEncoder()
    product_encoder_valid = LabelEncoder()
    y_train = product_encoder_train.fit_transform(y_train)
    y_valid = product_encoder_valid.fit_transform(y_valid)
...
# Test the neural network and evaluate its performance
def nn_testing():
    X_train, X_valid, y_train, y_valid, product_encoder_train, product_encoder_valid = create_train_valid_set()
    ...
    print("R2 Score on Valid Set:", r2_score(y_valid, y_valid_pred))
    print("MAE on Train Set:", train_mae)
    print("Percentage of MAE on Train Set:", (train_mae / len(y_train)) * 100)
    print("MAE on Valid Set:", valid_mae)
    print("Percentage of MAE on Valid Set:", (valid_mae / len(y_valid)) * 100)

# Final training of the neural network
def nn_final_training():
    X_train, _, y_train, _, product_encoder_train, _ = create_train_valid_set()

    model = neural_network(23, X_train.shape[1])
    model.fit(X_train, y_train, epochs=50, batch_size=16, verbose=0)
    model.save("final_model.h5")
    return product_encoder_train