...
def products_with_sales(num_of_sales=1000):
    ...
    if data.empty:
        print("No products were purchased more than {} times.".format(num_of_sales))
        return
...
def create_week_data():
    ...
    # Check if the 'order_timestamp' column exists
    if 'order_timestamp' not in data.columns:
        print("Error: 'order_timestamp' column not found.")
        return

    # Convert 'order_timestamp' to string type
    data["order_timestamp"] = data["order_timestamp"].astype("str")
    # Demand per week
...
def create_nn_data(number_of_weeks=16):
    ...
    for product in products:
        ...
        nn_data = pd.concat([nn_data, temp_data], ignore_index=True)  # Use concat instead of append

    nn_data.to_sql(name="nn_data", con=engine, index=False, if_exists="replace", chunksize=1000)
    # print_details(nn_data)
...
def pso_data():
    ...
    for product in products:
        ...
        pso = pd.concat([pso, temp], ignore_index=True)  # Use concat instead of append

    # Convert inf values to NaN
    pso.replace([np.inf, -np.inf], np.nan, inplace=True)
