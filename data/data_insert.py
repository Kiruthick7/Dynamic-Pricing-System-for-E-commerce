...
# Insert data into "sales2" table
sales2_data = pd.DataFrame({
    "sno_id": list(range(1, 3001)),
    "product_id": [f"101-{i}" for i in range(1, 3001)],
    "product_price": [15.5 + (i * 0.5) for i in range(3000)],
    "product_quantity": [1100 if i == 100 else (100 + (i * 10)) for i in range(3000)],  # Adjusted for a specific product    "customer_id": [f"101-{i}" for i in range(1, 3001)],
    "customer_id": [f"101-{i}" for i in range(1, 3001)],
    "order_id": [f"2001-{i}" for i in range(1, 3001)],
    "order_timestamp": pd.date_range("2024-04-01 08:30:00", periods=3000, freq="h")
})
sales2_data.set_index("sno_id", inplace=True)
sales2_data.to_sql(name="sales2", con=engine, if_exists="replace", index=True)
...
data_for_optimization = pd.DataFrame({
    "sno_id": list(range(1, 3001)),
    "product_id": [f"101-{i}" for i in range(1, 3001)],
    "arxikiTimi": [10.0 + (i * 0.5) for i in range(3000)],
    "telikiTimi": [9.5 + (i * 0.5) for i in range(3000)]
})
data_for_optimization.set_index("sno_id", inplace=True)
data_for_optimization.to_sql(name="data_for_optimization", con=engine, if_exists="replace", index=True)
